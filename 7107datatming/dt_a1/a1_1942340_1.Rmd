...
ggplot(afl2,aes(home,accuracy,fill=home))+
  geom_boxplot()
```
The home games have better probabilities to win more scores but the accuracy between home and away is very close. With the graph we can see the average line of home score is higher and the top and bottom is higher than away's. But when it comes to the accuracy graph their position is much closer.
